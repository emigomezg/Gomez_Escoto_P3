...
void GET_MSG_display(gmsg_profil_t terminal)
{
	...
	{
		case GMSG_TERMINAL0:
			if (FIFO_init(&msg0, GMSG_FIFO_SIZE) == FIFO_SUCCESS) {
				...
				PIT_callback_init(GET_MSG0_PIT_handler, PIT_1);
		...
		break;
		case GMSG_TERMINAL1:
			if (FIFO_init(&msg1, GMSG_FIFO_SIZE) == FIFO_SUCCESS) {
				...
				PIT_callback_init(GET_MSG1_PIT_handler, PIT_2);
...
void GET_MSG0_PIT_handler(void)
{
	...
	FIFO_push(&msg0, (hours / 10) + '0');
	FIFO_push(&msg0, (hours % 10) + '0');
	FIFO_push(&msg0, '/');
	FIFO_push(&msg0, (minutes / 10) + '0');
	FIFO_push(&msg0, (minutes % 10) + '0');
	FIFO_push(&msg0, '/');
	FIFO_push(&msg0, (seconds / 10) + '0');
	FIFO_push(&msg0, (seconds % 10) + '0');
	GET_MSG_send_to_UART(GMSG_TERMINAL0);
...
void GET_MSG1_PIT_handler(void)
{
	...
	uint8_t minutes = MCP7940M_get_minutes();
	uint8_t hours = MCP7940M_get_hours();
	FIFO_push(&msg1, (hours / 10) + '0');
	FIFO_push(&msg1, (hours % 10) + '0');
	FIFO_push(&msg1, '/');
	FIFO_push(&msg1, (minutes / 10) + '0');
	FIFO_push(&msg1, (minutes % 10) + '0');
	FIFO_push(&msg1, '/');
	FIFO_push(&msg1, (seconds / 10) + '0');
	FIFO_push(&msg1, (seconds % 10) + '0');
	GET_MSG_send_to_UART(GMSG_TERMINAL1);
...
void GET_MSG_send_to_UART(gmsg_profil_t terminal)
{
	switch ((uint8_t) terminal)
	{
		case GMSG_TERMINAL0:
			...
			while (msg0.status != FIFO_EMPTY) {
				UART_put_char(UART_0, FIFO_POP(&msg0));
			}

		break;
		case GMSG_TERMINAL1:
			UART_put_string(UART_1, &ret_pos[0]);
			while (msg1.status != FIFO_EMPTY) {
				UART_put_char(UART_1, FIFO_POP(&msg1));
			}
...
uint8_t GET_MSG_get_exit_flag(gmsg_profil_t terminal)
{
	...
	{
		case GMSG_TERMINAL0:
			if (get_msg0_exit_flag) PIT_disable_timer(PIT_1);
			return get_msg0_exit_flag;
		break;
		case GMSG_TERMINAL1:
			if (get_msg1_exit_flag) PIT_disable_timer(PIT_2);
